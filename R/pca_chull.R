...
pca_chull <- function(pca,axes=c(1,2), metadata= NULL, site.scaling = 1, species.scaling = 2, goodness.axis = 2, goodness.tresh = 0.3, main.group = NULL, second.group = NULL, nudge.x = NULL, nudge.y = NULL, scale.fill = NULL, scale.shape = NULL, scale.colour = NULL, labels = NULL, scale.col.labels = NULL, print.sp.tresh = FALSE, ext_plot_scale = 5){
...
#-----------------------------
  ...
  p_sites <- ggplot()
  if(is.null(second.group)){
    p_sites <- p_sites + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group), alpha=1, size = 2.5)
    }else{
...
#-----------------------------
  ...
  p_sp <- ggplot()
  if(is.null(second.group)){
     p_sp <- p_sp + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group), alpha=1, size = 2.5)
  }else{

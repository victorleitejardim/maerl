...
`autoplot.rda.chull` <- function(object, axes = c(1,2), geom = c("point", "text"),
                           layers = c("species", "sites", "biplot", "centroids"),
                           arrows = TRUE, legend.position = "right",
                           title = NULL, subtitle = NULL, caption = NULL, metadata = NULL, scale.fill = NULL, scale.colour = NULL, stat = NULL, thresh = 0.25, lvl = .95, ylab, xlab, const,title.size = 16, font.size = 18/.pt, ...) {
    ...
    var_axes <- as.numeric(round(object$CCA$eig/sum(object$CCA$eig)*100,2))
    if(missing(xlab)) {

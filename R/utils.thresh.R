...
`add_spp_site_scores` <- function(object, plt, vars, geom, draw_list, arrows, metadata, scale.fill, thresh, sp_fit) {
            ...
            if (geom == "point") {
                plt <- plt +
                    geom_point(data = object[take, , drop = FALSE],
                               aes_string(x = vars[1], y = vars[2],
                                          shape = 'Score', colour = group), size = 2.5)+
                ...
                plt <- plt +
                    geom_text_repel(data = object[take, , drop = FALSE ],
                              aes_string(x = vars[1], y = vars[2],
                                         label = 'Label'),
                              size = 3, segment.colour = "gray", max.overlaps = Inf)
            }
        } else {
    ...
    if (isTRUE(arrows) && draw_list["species"]) {
        ...
        plt <- plt + geom_text_repel(data = pdat,
                               aes_string(x = vars[1], y = vars[2],
                                          label = 'Label'), size = 4, fontface="bold", size = 9/.pt,  max.overlaps = Inf)
    }

...
`autoplot.rda` <- function(object, axes = c(1,2), geom = c("point", "text"),
                           ...
                           title = NULL, subtitle = NULL, caption = NULL, metadata = NULL, scale.fill = NULL,thresh = 0.25,
                           ylab, xlab, const, ...) {
    ...
    if (any(draw_list[c("species","sites","constraints")])) {
        plt <- add_spp_site_scores(obj, plt, vars, geom, draw_list, arrows, metadata, scale.fill = scale.fill, thresh = thresh, sp_fit = sp_fit)
    }
    ...
    ## do we want a legend
    plt <- plt + theme(legend.position = legend.position) + scale_colour_manual(values = scale.fill)
    plt
}

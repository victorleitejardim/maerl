...
pca_chull_y <- function(pca,axes=c(1,2), metadata= NULL, site.scaling = 1, species.scaling = 2, goodness.axis = 2, goodness.tresh = 0.3, main.group = NULL, second.group = NULL, nudge.x = NULL, nudge.y = NULL, scale.fill = NULL, scale.shape = NULL, scale.colour = NULL, labels = NULL, scale.col.labels = NULL, print.sp.tresh = FALSE, ext_plot_scale = 5){
...
#-----------------------------
  ...
  p_sites <- ggplot()
  if(is.null(second.group)){
    p_sites <- p_sites + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group), alpha=1, size = 2.5) + scale_y_continuous(limits = c(-0.3,0.3)) 
    }else{
    p_sites <- p_sites + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, shape=second.group), alpha=0.3)
}
  ...
  if(is.null(labels) & !is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = main.group, fill = main.group, colour = main.group), alpha= 1, fontface="bold", size = 12/.pt, inherit.aes = FALSE)
  }
  # If there are labels provided and a second grouping variable
  if(!is.null(labels) & !is.null(second.group)){
    p_sites <- p_sites + geom_label(data = label_pos, aes_string(x = "x", y = "y", label = "labels", fill = main.group, colour = main.group), alpha= 1, fontface="bold", size = 12/.pt, inherit.aes = FALSE)
  }
...
#-----------------------------
  ...
  p_sp <- ggplot()
  if(is.null(second.group)){
     p_sp <- p_sp + geom_point(data=site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, colour = main.group), alpha=1, size = 2.5) + scale_y_continuous(limits = c(-0.3,0.3)) 
  }else{
  ...
  p_sp <- p_sp + stat_chull(data = site_scores, aes_string(x = axes_name[1], y = axes_name[2], fill = main.group, col = main.group), inherit.aes = FALSE, geom="polygon", alpha=0.3, linetype = 2)
  ...
  p_sp <- p_sp + geom_text_repel(data=sp_scores[sp_scores$fit > goodness.tresh,], aes_string(x = axes_name[1], y = axes_name[2], label = "species"), fontface="bold", colour="black", segment.colour="black", max.overlaps = Inf)

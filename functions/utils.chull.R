...
`valid_layers.rda` <- function(object, ...) {
    c("species", "sites", "constraints", "biplot", "centroids", "regression", "fit")
}
##' @rdname valid_layers
##' @export
...
`add_spp_site_scores` <- function(object, plt, vars, geom, draw_list, arrows, metadata, scale.fill, scale.colour, thresh, sp_fit, stat, lvl, font.size, title.size) {
    ...
    if (length(wanted) > 0L) {
        ...
        if (length(geom) == 1L) {
            ...
            if (geom == "point") {
                ...
                plt <- plt +
                    geom_text_repel(data = object[take, , drop = FALSE ],
                              aes_string(x = vars[1], y = vars[2],
                                         label = 'Label'),
                              size = font.size, segment.colour = "gray80", colour = "gray80", max.overlaps = Inf, fontface = "italic")
            }
        } else {
                    ...
                    plt <- plt +
                        geom_text_repel(data = object[take, , drop = FALSE ],
                                  aes_string(x = vars[1],
                                             y = vars[2],
                                             label = 'Label'),
                                  size = font.size, colour = "gray80", segment.colour = "gray80", max.overlaps = Inf, fontface = "italic")
                }
            }
        }
        ...
        if (!is.null(stat)){
            if(stat == "chull"){
                plt <- plt + stat_chull(data = object[take, , drop = FALSE], aes_string(x = vars[1], y = vars[2], fill = group, col = group), geom = "polygon", alpha = 0.3, linetype = 2)
            } else {
                plt <- plt + stat_ellipse(data = object[take, , drop = FALSE], aes_string(x = vars[1], y = vars[2], fill = group, col = group), geom="polygon", alpha=0.3, linetype = 2, level = lvl)
            }
        }
            ...
            if (isFALSE(arrows)) {
                plt <- plt +
                    ...
                    geom_label_repel(data = pdat,
                                    ...
                                    max.overlaps = Inf,box.padding = .5
                                    )
    ...
    if (isTRUE(arrows) && draw_list["species"]) {
        ...
        plt <- plt +
            geom_segment(data = pdat,
                         ...
                         colour = col, size = 1, linetype = "dashed")+ theme(legend.position = "null")
        pdat[, vars] <- 1.1 * pdat[, vars, drop = FALSE]
        plt <- plt + geom_text_repel(data = pdat,
                               aes_string(x = vars[1], y = vars[2],
                                          label = 'Label'), size = font.size, fontface = "bold", colour = col, point.size = 5, max.overlaps = Inf, point.size = 2)
    }
